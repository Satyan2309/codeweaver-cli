...
export async function mapConnections({ frontend, backend }) {
  console.log(chalk.blue(`🔗 Mapping frontend (${frontend}) to backend (${backend})`));

  const frontendFiles = await globby(['**/*.js', '**/*.ts', '**/*.tsx'], { cwd: frontend, gitignore: true });
  const backendFiles = await globby(['**/*.js', '**/*.ts'], { cwd: backend, gitignore: true });
